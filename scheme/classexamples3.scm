...
(define keep-multiples
  (lambda (lst x)
    (filter (lambda (n) (= (remainder n x) 0)) lst)))
(keep-multiples lst 2)
(keep-multiples lst 3)
(keep-multiples (filter number? crazylist) 5)
...
; We can also use tail recursion
(define red-tail
  (lambda (operator base-case lst acc)
    (if (null? lst)
        acc
